...
- **Navbar (scroll-linked, no ScrollTrigger de sección):** el navbar en sí no tiene fondo propio en el estado inicial (scroll = 0) — el contraste del texto se resuelve con un **scrim del Hero**, no del navbar (ver detalle abajo). Al hacer scroll, el navbar se despega de los bordes superior y laterales (queda flotante, con margen), toma fondo oscuro (`#091821`) con `backdrop-filter: blur(...)` y esquinas redondeadas.
- **Scrim del Hero:** capa absoluta dentro de la sección Hero (no del header/navbar) que cubre aproximadamente los primeros 200-250px de la sección, con un degradé negro puro (`rgba(0,0,0,...)`) de tres stops (100% → 75% → 0%) para simular un contenedor de header con caída gradual. El texto del navbar en este estado es blanco (`#E8EEF1`) fijo. Esta capa se desvanece (`opacity: 1 → 0`) en el mismo rango de scroll donde aparece el navbar flotante, en crossfade simultáneo con la aparición del fondo oscuro+blur del navbar, para que la transición entre ambos estados sea continua y no deje ningún momento sin contraste. Se descartó la alternativa de que el degradé fuera parte del propio header (generaba problemas recurrentes de stacking/z-index entre el shade y el contenido en flujo del nav).
